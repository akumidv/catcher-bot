...
DEV_CACHE_PATH = os.path.normpath(os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../../.cache_data/MOEX'))

if IS_DEVELOP:
    os.makedirs(DEV_CACHE_PATH, exist_ok=True)


def dev_cache_async(func):
    @wraps(func)
    async def wrapped(*args, **kwargs):
        ...
        fn_path = None
        if IS_DEVELOP:
            fn_path = os.path.join(DEV_CACHE_PATH, f'{func.__name__}.parquet')
            if os.path.exists(fn_path):
                print(f'{func.__name__} from cache', fn_path)
...
class MoexOptions(Connector):
    ...
    @dev_cache_async
    async def get_symbol_list(self, underlying_type: Literal['futures', 'currency', 'share', 'index', 'commodity'] | None = None) -> pd.DataFrame:
        """
